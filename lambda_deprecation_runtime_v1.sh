...
aws lambda list-functions --profile $AWS_PROFILE --region $AWS_REGION --query "Functions[*].[FunctionName,Runtime]" --output text | while read -r line; do
    ...
    if is_runtime_deprecated "$runtime"; then
        ...
        application_tag=$(aws lambda list-tags --profile $AWS_PROFILE --region $AWS_REGION --resource "arn:aws:lambda:$AWS_REGION:$AWS_ACCOUNT_ID:function:$function_name" --query "Tags['Application System CI Name']" --output text)

        # Print the function details with the application tag
