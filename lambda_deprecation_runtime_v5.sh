...
# Create the header and footer lines based on the calculated column width
header_footer_line=$(printf '+-%-'$name_column_width's-+---------------------+------------------------------+\n' | tr ' ' '-')
...
# Read functions and runtimes into an array
IFS=$'\n' read -r -d '' -a functions_and_runtimes_array < <(aws lambda list-functions --profile $AWS_PROFILE --region $AWS_REGION --query "Functions[*].[FunctionName,Runtime]" --output text && printf '\0')

# Print table rows with dynamic spacing for function names
for line in "${functions_and_runtimes_array[@]}"; do
