...
# Store instance information for later processing
instance_info_array=()
while IFS= read -r instance_info; do
    instance_info_array+=("$instance_info")
done <<< "$instances"

# Calculate the maximum length for each column based on stored instance information
max_id_length=$(printf "%s\n" "${instance_info_array[@]}" | awk '{print $1}' | get_max_length)
max_ami_length=$(printf "%s\n" "${instance_info_array[@]}" | awk '{print $2}' | get_max_length)
max_launch_time_length=$(printf "%s\n" "${instance_info_array[@]}" | awk '{print $3}' | get_max_length)
...
# Step 2: Iterate through each instance and print the summary row
for instance_info in "${instance_info_array[@]}"; do
